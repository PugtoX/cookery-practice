...
| Image compression evidence | No longer "the site ships no raster images" — the redesign added the first files under `assets/img/`, and CR-9 added four more uses of them. **Corrected 2026-09-21:** that directory now holds **29 files / 4,045,583 bytes (3.86 MiB)**, not the "21 files / 3.9 MB" recorded here — the eight `-400` variants added in the CR-10 round were not counted when this row was written. The three-format `<picture>` (AVIF → WebP → JPEG) is itself the compression measure, and the figures are measured in `docs/redesign/photos.md`; what is still missing is a *before/after* number of the kind `portfolio` has (486 KB → 44.8 KB), because these were downloaded already-sized rather than compressed here. A second gap was found on 2026-09-21: the site's own per-image budget (≤100 KB) is unmet and no reason is written for the overages. Recorded in `web-gzliu/readiness-gate.md` G5. |
